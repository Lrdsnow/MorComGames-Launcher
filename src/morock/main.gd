...
var d = false
var morock = OS.get_executable_path().get_base_dir() + "/morock"
...
func installwin():
	...
	var morock_link = "https://github.com/Lrdsnow/morock/releases/download/v0.1test/morock.zip"
	...
	OS.execute("curl", ["https://github.com/Lrdsnow/morock/releases/latest/download/morock.zip", "-L", "-o", "%Appdata%\\mcgl\\morock.zip"])
	OS.execute("curl", ["https://raw.githubusercontent.com/Lrdsnow/MorComGames-Launcher/main/unzip.bat", "-L", "-o", "%Appdata%\\mcgl\\unzip.bat"])
	print("Downloaded!")
	d = false
	var cmd = '"Expand-Archive ' + "%AppData%\\mcgl\\morock.zip" + ' ' + morock_location + '"'
	print("unzipping")
	OS.execute("powershell.exe", ["-NoP", "-NonI", "-Command", '"Expand-Archive %AppData%\\mcgl\\morock.zip %Appdata%\\mcgl\\Games\\morock"'])
	print("Unzipped!")
